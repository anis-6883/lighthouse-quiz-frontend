      ...
      <div className='grid grid-cols-1 gap-y-8'>
        <div className='w-full'>
          ...
          {values?.siteLogo ? (
            ...
            <ImageDropzone value={siteLogo} onChange={(image: any) => setSiteLogo(image)} className='mt-2' size={1024 * 1000} sizeText='1MB' />
          )}
        </div>
          ...
          {values?.siteIcon ? (
            <div className='flex items-center gap-3'>
              <Image
                ...
                className='h-24 w-24 rounded-md border border-gray-200 object-contain p-1'
              />
              <button type='button' className='rounded bg-red-500 p-1' onClick={() => setFieldValue('siteIcon', '')}>
                <FaTrashAlt className='hover:fill-secondary-400 h-5 w-5 fill-white transition-colors' />
              </button>
            </div>
          ) : (
            <ImageDropzone value={siteIcon} onChange={(image: any) => setSiteIcon(image)} className='mt-2' size={1024 * 1000} sizeText='1MB' />
          )}
        </div>
      </div>
    </FormBlockWrapper>
