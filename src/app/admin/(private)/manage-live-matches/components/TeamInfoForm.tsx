                ...
                <label className='form-control w-full'>
                  <div className='label'>
                    <span className='label-text font-semibold'>
                      Name <span className='text-red-600'>* {meta.touched && meta.error && <span>({meta.error})</span>}</span>
                    </span>
                  </div>
              ...
              {({ field, meta }: { field: any; meta: any }) => (
                <>
                  <label className='form-control w-full'>
                    ...
                    <input type='url' className={`input input-bordered w-full ${meta.touched && meta.error && 'input-error'}`} {...field} />
                  </label>
                </>
              )}
            </Field>
          ...
          {values?.team_one_image_type === 'image' && values.team_one_image && (
            <div className='mt-2 flex items-center gap-3'>
              <Image
                ...
                className='h-24 w-24 rounded-md border border-gray-200 object-contain p-1'
              />
              <button type='button' className='rounded bg-red-500 p-1' onClick={() => setFieldValue('team_one_image', '')}>
          ...
          {values?.team_two_image_type === 'image' && values.team_two_image && (
            <div className='mt-2 flex items-center gap-3'>
              <Image
                ...
                className='h-24 w-24 rounded-md border border-gray-200 object-contain p-1'
              />
              <button type='button' className='rounded bg-red-500 p-1' onClick={() => setFieldValue('team_two_image', '')}>
                <FaTrashAlt className='hover:fill-secondary-400 h-5 w-5 fill-white transition-colors' />
              </button>
            </div>
          )}
